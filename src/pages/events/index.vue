    ...
    <section class="block --mt-large">
      ...
      <div class="container">
        ...
        <div class="col --main">
          ...
          <article :key="event.id" v-for="event in oldEvents">
            <div class="block --full --mb">
              <event :event="event" />
            </div>
          </article>
...
export default {
  ...
  methods: {
    getPageDetails() {
      db.getEntryById("645Q1Gtp3UiYEsK0Kiys8Q").then(response => {
        this.loading = false;
        this.page = response;
      });
    },
    getUpcoming() {
      this.loading = true;
      db.getUpcomingEvents(100, 0).then(response => {
        this.loading = false;
    ...
    getAll() {
      this.loading = true;
      db.getEntries("event", 100, 0).then(response => {
        this.loading = false;
        this.events = response;
      });
    }
